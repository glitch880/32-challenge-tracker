...
- **Cite names, not line numbers.** Point at `renderArt()` or `STAMPED`, never `index.html:574`. An anchor into a 780-line file goes stale the next time anything above it changes, and nothing complains — this file drifted three PRs running that way before the rule existed. A bare `[index.html](index.html)` link is fine; a `:NN` on the end is not, and `node --test` fails on one.
