...
`index.html`'s `checkScripts()` names every symbol it expects from the two scripts so a stale-cache load reports which one is missing instead of dying quietly. **Add new exported symbols to that list** — a symbol that is exported and used but unlisted is invisible to exactly the failure the check exists to catch.
...
`rowId` is an identity id from `IDENTITIES` (`'gwu'`, `'c'`, `'wubrg'`). Per-person nodes are separate so two people editing at once never clobber each other. **A new per-deck field means another sibling node — and it must also be deleted in `removePerson()`, which removes each node by hand.**
...
- **Scryfall query syntax lives in `logic.js`, not at the call sites** — `commanderQuery()` (autocomplete) and `printsQuery()` (exact-name match; the caller adds `&unique=prints`) build the `q=` payload, and `imageUrl()` builds card-image URLs, choosing pinned id vs. fuzzy name. The two `/cards/search` endpoint URLs are assembled inline where they're fetched, in `suggest()` and `openPrintPicker()`; the query *syntax* is what must not be re-derived there. The worked example: `commanderQuery` uses `id=` (exact), never `id<=`, because subset matching lets mono cards into a 3-color row.
- **Re-render guards are load-bearing.** `root.on('value')` fires on every remote edit, so both per-person views re-render constantly: they rebuild DOM only when the person changed — `builtFor` for `renderSheet()`, `artFor` for `renderArt()` — and `renderArt()` assigns `img.src` only when its `img.dataset.want` check sees the URL actually change — without that, each snapshot re-requests all 32 images. Field values also skip `document.activeElement`, so a live update can't yank the cursor out of what someone is typing.
...
- **Cite names, not line numbers.** Point at `renderArt()` or `STAMPED`, never `index.html:574`. An anchor into a file this size goes stale the next time anything above it changes, and nothing complains — this file drifted three PRs running that way before the rule existed. A bare `[index.html](index.html)` link is fine; a trailing `:NN` or `#L NN` is not. `node --test` catches those in markdown links, but it can't see one written in prose — so don't write one.
